...
func TestIgnoreLameReferrals(t *testing.T) {
	...
	addrs, _, err := r.LookupIP(context.Background(), "ip4", "www.golang.org")
	if err != nil {
		t.Fatal(err)
	}
	...
	_, _, err = r.LookupIP(context.Background(), "ip4", "empty.com")
	de, ok := err.(*DNSError)
	if !ok {
...
func TestNullMX(t *testing.T) {
	...
	if want := []*net.MX{{Host: "."}}; !reflect.DeepEqual(rrset, want) {
		records := []string{}
...
func TestRootNS(t *testing.T) {
	...
	if want := []*net.NS{{Host: "i.root-servers.net."}}; !reflect.DeepEqual(rrset, want) {
		records := []string{}
		for _, rr := range rrset {
			records = append(records, fmt.Sprintf("%v", rr))
		}
